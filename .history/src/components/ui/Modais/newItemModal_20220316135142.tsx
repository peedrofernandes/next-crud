...
export default function NewItemModal(props: ModalProps) {
  ...
  const [properties, setProperties] = useState<NewObjectPropertiesList[]>([]);
            ...
            return (
              <div className={styles.inputs} key={i}>
                ...
                <div className={styles.inputAndSelect}>
                  ...
                  <input onChange={(e) => handleSetPropertyName(e.target.value, i)} type="text" placeholder={`Nome`}></input>
                </div>
              </div>
            )
            })
