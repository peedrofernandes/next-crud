...
export default function NewItemModal(props: ModalProps) {
  ...
  function handleRemoveInput(e) {
    ...
    setInputTypes(JSON.parse(JSON.stringify(inputTypes)).slice(0, -1))
  }
                ...
                <div className={styles.inputAndSelect}>
                  {
                    ...
                    ) : inputTypes[i] === 'boolean' ? (
                      <select>
                        <option value="true">True</option>
                        <option value="false">False</option>
                      </select>
                    ) : (
                      <input type="text" placeholder="Selecione uma opção!" disabled></input> 
                    )
                  }
                  <select onChange={(ev) => handleChangeInputType(i, ev.target.value)} placeholder="Tipo">
